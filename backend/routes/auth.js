...
router.post('/register', [
  body('username').trim().isLength({min:3,max:50}).matches(/^[a-zA-Z0-9_]+$/).withMessage("Nom d'utilisateur invalide"),
  body('email').trim().isEmail().normalizeEmail().withMessage('E-mail invalide'),
  body('password').isLength({min:8}).matches(/[A-Z]/).matches(/[0-9]/).withMessage('Mot de passe trop faible'),
  ...
  try {
    if (await User.emailExists(email)) return res.status(409).json({ errors:[{msg:'E-mail déjà utilisé'}] });
    if (await User.usernameExists(username)) return res.status(409).json({ errors:[{msg:"Nom d'utilisateur déjà pris"}] });
  ...
  } catch(err) { console.error(err); return res.status(500).json({ errors:[{msg:'Erreur serveur'}] }); }
});
